...
// ─── Maison Merch: World Cup Hub — countdown + live scores + standings
// Reads the template file directly so no complex string ops are needed.
add_action( 'init', function() {
	if ( get_option( 'mm_wc_scores_v3' ) === '1' ) {
		return;
	}
	global $wpdb;
...
// Maison Merch: Fix countdown strip - self-contained JS, no external API (v4)
add_action( 'init', function() {
	if ( get_option( 'mm_wc_scores_v4' ) === '1' ) return;
	global $wpdb;
	$theme_dir = get_template_directory();
	$templates = array(
		'front-page' => $theme_dir . '/templates/front-page.html',
	...
	wp_cache_flush();
	update_option( 'mm_wc_scores_v4', '1' );
} );

// Maison Merch: Re-sync templates after cleaning multibyte chars from JS (v5)
add_action( 'init', function() {
	if ( get_option( 'mm_wc_scores_v5' ) === '1' ) return;
	global $wpdb;
	$theme_dir = get_template_directory();
	$templates = array(
		'front-page' => $theme_dir . '/templates/front-page.html',
...
// Maison Merch: Update countdown label to 'next match starts in' (v6)
add_action( 'init', function() {
...
// Maison Merch: Countdown label -> 'next match starts in...' (v7)
add_action( 'init', function() {
	if ( get_option( 'mm_wc_scores_v7' ) === '1' ) return;
	global $wpdb;
	$theme_dir = get_template_directory();
	$templates = array(
		'front-page' => $theme_dir . '/templates/front-page.html',
	...
	wp_cache_flush();
	update_option( 'mm_wc_scores_v7', '1' );
} );

// Maison Merch: Fix countdown strip spacing - wrap left cluster (v8)
add_action( 'init', function() {
	if ( get_option( 'mm_wc_scores_v8' ) === '1' ) return;
	global $wpdb;
	$theme_dir = get_template_directory();
	$templates = array(
		'front-page' => $theme_dir . '/templates/front-page.html',
	...
	wp_cache_flush();
	update_option( 'mm_wc_scores_v8', '1' );
} );
...
// Maison Merch: Remove World Cup section and WC copy from homepage (v1)
add_action( 'init', function() {
	if ( get_option( 'mm_remove_wc_v1' ) === '1' ) return;
	global $wpdb;
	$theme_dir = get_template_directory();
	$templates = array(
		'front-page' => $theme_dir . '/templates/front-page.html',
	...
	wp_cache_flush();
	update_option( 'mm_remove_wc_v1', '1' );
} );

// Maison Merch: Remove announcement bar from header template part (v1)
add_action( 'init', function() {
	if ( get_option( 'mm_remove_announce_v1' ) === '1' ) return;
	global $wpdb;

	// Get the header template part post
	$post = $wpdb->get_row(
		"SELECT ID, post_content FROM {$wpdb->posts}
	...
	if ( $post && strpos( $post->post_content, 'announcement-bar' ) !== false ) {
		// Strip the entire announcement-bar div block
		$cleaned = preg_replace(
			'/<div[^>]*class=["\'][^"\']*announcement-bar[^"\']*["\'][^>]*>.*?<\/div>\s*/si',
			'',
			$post->post_content
		);
		if ( $cleaned && $cleaned !== $post->post_content ) {
			$wpdb->update(
				$wpdb->posts,
...
// Maison Merch: Remove announcement bar from DB header - debug + broader search (v3)
add_action( 'init', function() {
	if ( get_option( 'mm_remove_announce_v3' ) === '1' ) return;
	global $wpdb;

	// Search ALL posts containing announcement-bar (regardless of type/name)
	$posts = $wpdb->get_results(
		"SELECT ID, post_content, post_type, post_name FROM {$wpdb->posts}
		 WHERE post_content LIKE '%announcement-bar%'
		 AND post_status = 'publish'"
	);
	...
	foreach ( $posts as $post ) {
		...
		$div_open = strrpos( substr( $content, 0, $marker ), '<div' );
		if ( $div_open === false ) continue;

		$pos = $div_open; $depth = 0; $end = null; $len = strlen( $content );
		while ( $pos < $len ) {
			if ( substr( $content, $pos, 4 ) === '<div' ) {
				$depth++; $pos += 4;
			} elseif ( substr( $content, $pos, 6 ) === '</div>' ) {
		...
		if ( $end !== null ) {
			$cleaned = substr( $content, 0, $div_open ) . ltrim( substr( $content, $end ) );
			$wpdb->update( $wpdb->posts, array( 'post_content' => $cleaned ), array( 'ID' => $post->ID ) );
		}
	}

	wp_cache_flush();
	update_option( 'mm_remove_announce_v3', '1' );
} );
